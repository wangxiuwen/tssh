...
`
	script = strings.ReplaceAll(script, "__SVC__", shellQuote(svc))
	script = strings.ReplaceAll(script, "__NSFLAG__", nsFlag)
	// __NS__ last because it could otherwise match fragments.
	script = strings.ReplaceAll(script, "__NS__", shellQuote(namespace))

	fmt.Fprintf(os.Stderr, "🔍 在 %s 查询 k8s service %s (ns=%s)...\n", inst.Name, svc, defaultStr(namespace, "default"))
	res, err := client.RunCommand(inst.ID, script, 30)
	fatal(err, "run diag script")
	output := decodeOutput(res.Output)

	diag := parseKSOutput(output)
	diag.Jump = inst.Name
...
func printKSResult(d ksDiag) {
	...
	for _, p := range d.Pods {
		...
		fmt.Printf("   %-30s %-10s %-5s %-14s %8s %8s %8d %8d\n",
			truncateStr(p.Name, 30), p.Phase, ready, p.IP,
			defaultStr(p.CPU, "-"), defaultStr(p.Mem, "-"),
			p.Estab, p.TW)
	}
	fmt.Println()
...
// defaultStr returns dflt when s is empty, otherwise s — avoids "<none>"
