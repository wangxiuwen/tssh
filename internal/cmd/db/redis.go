...
// appRuntime is set via Redis(rt, args) — the Arms-style entry point.
...
func cmdRedisLs(args []string) {
	...
	client, err := aliyun.NewRedisClient(config)
	shared.Fatal(err, "create Redis client")

	fmt.Fprintln(os.Stderr, "🔄 正在从阿里云拉取 Redis 实例列表...")
	instances, err := client.FetchAllRedisInstances()
	shared.Fatal(err, "fetch Redis instances")
...
func cmdRedisConnect(args []string) {
	...
	config := appRuntime.LoadConfig()
	redisClient, err := aliyun.NewRedisClient(config)
	shared.Fatal(err, "create Redis client")
	...
	// 2. Find an ECS jump host in the same VPC — use runtime cache access.
	...
	socatPort := 19900
	socatCmd := fmt.Sprintf("nohup socat TCP-LISTEN:%d,fork,reuseaddr TCP:'%s':%d &>/dev/null & echo $!", socatPort, shared.ShellQuote(found.ConnectionDomain), remotePort)
