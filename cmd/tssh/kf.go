...
func cmdKF(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		case "-n", "--namespace":
			if i+1 >= len(args) {
				fmt.Fprintln(os.Stderr, "❌ -n 需要 namespace")
				os.Exit(2)
			}
			namespace = args[i+1]
			i++
	...
	for i := range targets {
		if targets[i].remotePort == 0 {
			...
			targets[i].remotePort = findFreePortInRange(18000, 18999)
		}
		startCmd := fmt.Sprintf(
			"nohup kubectl%s port-forward --address 127.0.0.1 svc/%s %d:%d >/tmp/tssh-kf-%d.log 2>&1 & echo $!",
			nsFlag, shellQuote(targets[i].svc), targets[i].remotePort, targets[i].svcPort, targets[i].remotePort,
		)
		res, err := client.RunCommand(inst.ID, startCmd, 15)
		if err != nil {
			rollback()
			fatal(err, fmt.Sprintf("kubectl port-forward for %s", targets[i].svc))
		}
		pid := strings.TrimSpace(decodeOutput(res.Output))
		if _, perr := strconv.Atoi(pid); perr != nil {
			rollback()
			fatalMsg(fmt.Sprintf("启动 kubectl 失败 (svc=%s): 可能 kubectl 未安装或 kubeconfig 没配", targets[i].svc))
		}
		...
		cleanups = append(cleanups, func() {
			_, _ = client.RunCommand(inst.ID, fmt.Sprintf("kill %s 2>/dev/null", shellQuote(pidCopy)), 5)
		})
		...
		stop, err := startPortForwardBgWithCancel(cfg, inst.ID, targets[i].localPort, targets[i].remotePort)
		if err != nil {
			rollback()
			fatal(err, fmt.Sprintf("port-forward for %s", targets[i].svc))
		}
...
// openURL launches the default browser on the host platform. Best-effort;
// a failure here shouldn't block the tssh kf tunnels from staying up.
func openURL(url string) error {
	// execLookPath lives in info.go; reuse it.
	candidates := []string{"open", "xdg-open"}
	for _, bin := range candidates {
		if path, err := execLookPath(bin); err == nil {
			return execCommand(path, url).Start()
		}
	}
