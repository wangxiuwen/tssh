...
func VPN(rt core.Runtime, args []string) {
	cidrList := ""
	tunName := ""
	var target string
	var jsonMode bool
	...
	if runtime.GOOS != "linux" && runtime.GOOS != "darwin" {
		shared.FatalMsg("tssh vpn 目前只支持 Linux / macOS (当前: " + runtime.GOOS + ")")
	}
	if os.Geteuid() != 0 {
		shared.FatalMsg("tssh vpn 需要 root (添加路由 + 创建 TUN 设备)\n   请用: sudo tssh vpn ...")
	}
	tun2socksBin, err := exec.LookPath("tun2socks")
	if err != nil {
		shared.FatalMsg(`未找到 tun2socks 二进制. 安装方式:
...
sudo 下 PATH 可能被重置, 可用: sudo env "PATH=$PATH" tssh vpn ...`)
	...
	socksPID, err := session.StartRemoteSocks(client, inst.ID, socksRemotePort)
	shared.Fatal(err, "start microsocks")
	cleanupSocks := func() {
		_, _ = client.RunCommand(inst.ID, fmt.Sprintf("kill %s 2>/dev/null", shared.ShellQuote(socksPID)), 5)
	}

	// --- 2. Local port-forward to microsocks ---
	socksLocalPort := shared.FindFreePort()
	stopPF, err := rt.StartPortForward(inst.ID, socksLocalPort, socksRemotePort)
	if err != nil {
		cleanupSocks()
	...
	if err := t2sCmd.Start(); err != nil {
		stopPF()
		cleanupSocks()
		shared.Fatal(err, "start tun2socks")
	}
	...
	if err := waitForTun(tunName, 5*time.Second); err != nil {
		...
		stopPF()
		cleanupSocks()
		shared.Fatal(err, "wait for tun device")
	}
